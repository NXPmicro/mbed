...
#define QCA400X_SOCKET_COUNT 5

#define QCA400X_INTERFACE_CONNECT_INTERVAL_MS (5000)
#define QCA400X_SSID_MAX_LENGTH 32
#define QCA400X_PASSPHRASE_MAX_LENGTH 63
#define QCA400X_PASSPHRASE_MIN_LENGTH 8  /* The shortest allowed passphrase */

/** QCA_WiFiInterface interface class
 *  Implementation of the NetworkStack for QCA 4002/4004
 */
class QCA_WiFiInterface: public NetworkStack, public WiFiInterface {
public:
    /**
     * @brief QCA_WiFiInterface default constructor
     */
    ...
    /**
     * @brief QCA_WiFiInterface default destructor
     */
    virtual ~QCA_WiFiInterface();
    ...
    virtual nsapi_error_t connect();
     ...
     /** Set the WiFi network channel - if the AP doesn't support this, return not supported
       *
       *
       *  @param channel   Channel on which the connection is to be made, or 0 for any (Default: 0)
       *  @return          0 on success, or error code on failure
...
protected:
     ...
     *  @return             0 on success, negative on failure.
     */
    virtual int socket_bind(void *handle, const SocketAddress &address);         // not required by PDMC
     ...
     *  @return             0 on success, negative on failure
     */
    virtual int socket_listen(void *handle, int backlog);                       // not required by PDMC
     ...
     */
    virtual int socket_accept(void *handle, void **socket, SocketAddress *address); // not required by PDMC
     ...
     *  @param handle       Socket handle
     *  @param addr         Destination for the remote SocketAddress or null
     *  @param data         The buffer for storing the incoming packet data
...
private:
    ...
    struct _sock_info _sock_i[QCA400X_SOCKET_COUNT];

    /* Add other private members here */
    int _initialized;
    nsapi_error_t _connect_retval;

    char _ssid[QCA400X_SSID_MAX_LENGTH + 1];
    char ap_pass[QCA400X_PASSPHRASE_MAX_LENGTH + 1]; /* The longest possible passphrase; +1 for the \0 */
